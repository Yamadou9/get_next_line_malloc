...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/26 15:26:22 by ydembele          #+#    #+#             */
/*   Updated: 2025/05/26 15:41:31 by ydembele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd > 1024 || fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (free(tab[fd]), tab[fd] = NULL, NULL);
	if (!tab[fd])
	{
		tab[fd] = malloc(sizeof(char) * (BUFFER_SIZE + 1));
		if (!tab[fd])
			return (NULL);
		tab[fd][0] = '\0';
	}
	...
	if (!buffer)
		return (free(tab[fd]), tab[fd] = NULL, NULL);
	next = next_line(tab[fd], fd, buffer);
	if (!next || tab[fd][0] == 0)
	{
		free(tab[fd]);
		tab[fd] = NULL;
